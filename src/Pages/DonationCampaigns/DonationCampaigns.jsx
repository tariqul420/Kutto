    ...
    <div className="mb-20 mt-6">
      <div className="w-11/12 mx-auto">
        ...
        <div className="mb-6 flex gap-4 justify-between items-center">
          <input type="text" placeholder="Search by name..." className="inputField p-2 w-1/3 max-sm:w-full" value={search} onChange={(e) => setSearch(e.target.value)} />
          <select className="inputField p-2 w-1/3 max-sm:w-full" value={sort} onChange={handelSortChange}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {isLoading ? (
            ...
            pets?.pages.flat().map((donation) => <DonationCard key={donation?._id} donation={donation} />)
          )}
        </div>
      </div>
      ...
      {!hasNextPage && pets?.pages.flat().length > 0 && <p className="text-center text-gray-500 mt-4">No more donations to load.</p>}
    </div>
  );
};
