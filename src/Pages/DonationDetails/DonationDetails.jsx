...
const DonationDetails = () => {
  ...
  const {
    data: suggestedDonation = [],
    isLoading: isLoadingSuggested,
    refetch: isRefetchSuggested,
  } = useQuery({
  ...
  return (
    <section>
      ...
      <div className="donation-details w-11/12 mx-auto flex flex-col lg:flex-row gap-8 my-16">
        <div className="flex-1">
          <img src={donation?.donationImage} alt={donation?.donationName} className="w-full h-[400px] object-cover rounded-md" />
        </div>
        ...
        <div className="flex-1 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p>
                <strong>maxAmount:</strong> $ {donation?.maxAmount}
              </p>
              <p>
                <strong>Donated Amount:</strong> $ {donation?.totalDonateAmount}
              </p>
              <p>
                <strong>Last Date:</strong>{' '}
                {new Date(donation?.lastDate).toLocaleDateString('en-US', {
                  day: '2-digit',
                  month: 'long',
                  year: 'numeric',
                })}
              ...
              <p>
                ...
                <span
                  className={`${donation?.status === 'Running' && 'text-green-600'} ${donation?.status === 'Pause' && 'text-red-600'} ${
                    donation?.status === 'Complete' && 'text-blue-600 font-semibold'
                  }`}>
                  {donation?.status}
                </span>
              </p>
              {new Date() > new Date(donation?.lastDate) && <p className="text-red-500">The donation campaign has ended.</p>}
            </div>
            <div>
              <CircleProgress progress={(donation?.totalDonateAmount / donation?.maxAmount) * 100 || 0}>
                ...
                <CircleProgressText>{Math.round((donation?.totalDonateAmount / donation?.maxAmount) * 100 || 0)}%</CircleProgressText>
              </CircleProgress>
            </div>
          </div>

          <p>
            <strong>Description:</strong> {donation?.shortDescription}
          </p>
          ...
          <Elements stripe={stripePromise}>
            <Modal open={isModalOpen} onOpenChange={setIsModalOpen}>
              <ModalAction asChild>
                <Button
                  disabled={
                    donation?.status === 'Pause' || donation?.maxAmount === donation?.totalDonateAmount || new Date() > new Date(donation?.lastDate) || user?.email === donation?.donationOwner?.email
                  }
      ...
      <div className="w-11/12 mx-auto my-20">
        ...
        <div className="donation-description leading-relaxed mt-8" dangerouslySetInnerHTML={{ __html: sanitizedDescription }} />
      </div>
      ...
      <div className="w-11/12 mx-auto mb-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-8">
          {isLoadingSuggested
            ...
            : suggestedDonation?.map((donation) => <DonationCard key={donation?._id} donation={donation} isRefetchSuggested={isRefetchSuggested} />)}
        </div>
      </div>
    </section>
  );
