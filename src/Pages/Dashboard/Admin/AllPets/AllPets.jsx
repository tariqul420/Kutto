  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        header: 'Image',
        cell: (info) => <img src={info.getValue()} alt={info.row.original.petName} className="w-12 h-12 object-cover rounded-md" />,
      },
      {
        accessorKey: 'petName',
        header: 'Pet Name',
      ...
      },
      {
        ...
        cell: (info) => (
          <div className="flex gap-2 flex-wrap justify-center">
            ...
            <button className="bg-red-500 p-1 rounded-md text-white" onClick={() => handleDelete(info.row.original._id)}>
              <RiDeleteBin5Fill />
            </button>
            <button className={`p-1 rounded-md text-white ${info.row.original.adopted ? 'bg-red-500' : 'bg-green-500'}`} onClick={() => handleAdopt(info.row.original._id, info.row.original.adopted)}>
