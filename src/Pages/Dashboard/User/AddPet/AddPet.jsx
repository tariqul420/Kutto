...
const AddPet = () => {
  ...
  const getCustomStyles = (theme) => ({
    ...
    option: (base, state) => ({
      ...base,
      backgroundColor: state.isSelected ? (theme === 'dark' ? '#1d2a39' : '#007bff') : state.isFocused ? (theme === 'dark' ? '#2a3948' : '#e9ecef') : theme === 'dark' ? '#1b1f26' : '#fff',
      color: state.isSelected ? '#fff' : theme === 'dark' ? '#e0e0e0' : '#495057',
      cursor: 'pointer',
    }),
    menu: (base) => ({
      ...base,
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="w-full flex flex-col gap-5">
          ...
          <div className="flex flex-col lg:flex-row gap-4">
            ...
            <div className="flex-1">
              <input type="text" placeholder="Pet Name" className="inputField w-full" {...register('petName', { required: 'Pet name is required.' })} />
              {errors.petName && (
            ...
            <div className="flex gap-4 flex-1">
              <div className="flex-1">
                <input type="number" placeholder="Pet Age" className="inputField w-full" {...register('petAge.value', { required: 'Pet age is required.' })} />
                {errors.petAge?.value && (
                  <p className="flex text-red-500 gap-1 items-center">
              ...
              <div>
                <select className="inputField w-full" {...register('petAge.unit', { required: 'Pet age unit is required.' })}>
          ...
          <div className="flex flex-col lg:flex-row gap-4">
            {/* Pet Location */}
            <div className="flex-1">
              <input type="text" placeholder="Pet Location" className="inputField w-full" {...register('petLocation', { required: 'Pet location is required.' })} />
              {errors.petLocation && (
                <p className="flex text-red-500 gap-1 items-center">
          ...
          {/* Short Description */}
          <textarea rows={2} placeholder="Short Description" className="inputField w-full" {...register('shortDescription', { required: 'Short description is required.' })} />
          ...
          {/* Long Description */}
          <div>
            <ReactQuill theme="snow" value={longDesc} onChange={setLongDesc} className={`border ${longDescError ? 'border-red-500' : ''}`} />
            {longDescError && (
          ...
          {/* Pet Photo */}
          <label htmlFor="petImage" className="flex items-center justify-center w-full h-16 border-2 border-dashed border-gray-300 rounded-md cursor-pointer hover:border-color-accent transition-all">
            <span className="text-gray-500 flex items-center gap-2">
              <MdCloudUpload className="text-xl" /> Upload Photo
            </span>
            {photoPreview && <img src={photoPreview} alt="Preview" className="w-14 h-14 object-cover rounded-md border border-gray-300 ml-4" />}
            <input
              id="petImage"
              type="file"
              accept="image/*"
          ...
          <div className="w-full flex items-center justify-center">
            <button type="submit" className="inputButton w-full lg:w-1/2">
