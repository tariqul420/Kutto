  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        header: 'Image',
        cell: (info) => <img src={info.getValue()} alt={info.row.original.donationImage} className="w-12 h-12 object-cover rounded-md" />,
      },
      {
      ...
      },
      {
        ...
        cell: (info) => (
          <LineProgress progress={(info.row.original.totalDonateAmount / info.row.original.maxAmount) * 100 || 0}>
            ...
            <LineProgressText className="text-error-500">{Math.round((info.row.original.totalDonateAmount / info.row.original.maxAmount) * 100 || 0)}%</LineProgressText>
          </LineProgress>
        ),
      },
      {
        accessorKey: 'status',
        header: 'Status',
        cell: (info) => <span className={`${info.row.original.status === 'Running' ? 'text-green-500' : 'text-red-500'}`}>{info.getValue()}</span>,
      },
      {
        id: 'actions',
        header: 'Actions',
        cell: (info) => (
          <div className="flex flex-col gap-2">
            <button
              disabled={info.row.original.status === 'Complete' || info.row.original.maxAmount === info.row.original.totalDonateAmount}
              ...
              <ModalContent className="max-w-[20rem] lg:max-w-[26rem]">
                <ModalHeader className="mb-6 flex flex-col items-center justify-center space-y-3">
                  ...
                  <div className="space-y-1 text-center">
                    ...
                    <ModalDescription>
                      Total Donation: <span className="text-color-accent">${info.row.original.totalDonateAmount}</span>, Total Donators:{' '}
                      <span className="text-color-accent">{info.row.original.totalDonateUser} Members</span>
                    </ModalDescription>
                  </div>
                </ModalHeader>
              </ModalContent>
  ...
  return (
    <div className="overflow-x-auto">
          ...
          {table.getHeaderGroups().map((headerGroup) => (
            <tr key={headerGroup.id}>
              {headerGroup.headers.map((header) => (
                <th key={header.id} className="border border-gray-300 px-4 py-2 cursor-pointer" onClick={header.column.getToggleSortingHandler()}>
      ...
      <div className="flex justify-between items-center mt-4 flex-wrap">
        ...
        <button className="bg-color-accent px-2 py-1 rounded-md text-white disabled:cursor-not-allowed disabled:bg-gray-600" onClick={() => table.nextPage()} disabled={!table.getCanNextPage()}>
          Next
        </button>
      </div>
    </div>
