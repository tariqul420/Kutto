...
const Dashboard = () => {
  ...
  return (
    <div className="flex relative gap-1 dashboard overflow-hidden">
      ...
      {/* Sidebar */}
      <div
        className={`fixed inset-y-0 left-0 z-40 transform bg-white dark:bg-black transition-transform duration-300 ease-in-out md:static md:transform-none ${
        ...
        <div className="sidebar-scrollable">
          <nav className="w-full sticky top-0 z-[50] bg-white dark:bg-dark-lite transition-transform duration-300 mb-2 py-1">
            <div className="w-11/12 mx-auto flex items-center justify-between">
              <div>
                <input type="text" placeholder="Search by name..." className="inputField p-2 w-full " />
              </div>

              <div className="items-center gap-[15px] flex">
                <button onClick={toggleTheme} className="p-2 rounded-full bg-white dark:bg-gray-700">
                  {theme === 'dark' ? <IoMdSunny size={20} color="#F04335" /> : <MdDarkMode size={20} color="#1E1E1E" />}
                </button>
