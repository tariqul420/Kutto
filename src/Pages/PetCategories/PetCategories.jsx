    ...
    <section>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-8 w-11/12 mx-auto">
        {isLoading ? (
          ...
          petByCategory.pages.flat()?.map((pet) => <PetCard key={pet?._id} pet={pet} />)
        )}
      </div>

      {!hasNextPage && petByCategory?.pages.flat().length > 0 && <p className="text-center text-gray-500 my-6">No more pets to load.</p>}
    </section>
  );
};
