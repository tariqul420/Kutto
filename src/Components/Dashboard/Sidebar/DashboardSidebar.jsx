...
import { Avatar, AvatarFallback, AvatarImage, Divider, Sidebar, SidebarBody, SidebarFooter, SidebarItem, SidebarList } from 'keep-react';
    ...
    <Sidebar className="mt-2 max-h-[100vh] h-full overflow-auto w-full">
      <SidebarBody className="sidebar-scrollable">
        <div className="flex justify-between">
          ...
          <div onClick={handleLinkClick} className="md:hidden">
            <XCircle size={24} />
          </div>
        </div>
      ...
      <SidebarFooter>
        <div className="flex items-center gap-2">
          <Avatar>
            {user?.photoURL ? (
              <AvatarImage referrerPolicy="no-referrer" src={user.photoURL} alt="avatar" className="object-cover" />
            ) : (
              <AvatarFallback>{user?.displayName ? user.displayName.slice(0, 2).toUpperCase() : 'NA'}</AvatarFallback>
            )}
          </Avatar>
          <div>
            <p className="text-body-4 font-medium text-metal-400">{user?.displayName?.length < 14 ? user.displayName : user.displayName.substring(0, 11) + '...'}</p>
            <p className="text-body-4 font-normal text-metal-300">{user?.email?.length < 17 ? user.email : user.email.substring(0, 17) + '...'}</p>
          </div>
        </div>
      </SidebarFooter>
    </Sidebar>
